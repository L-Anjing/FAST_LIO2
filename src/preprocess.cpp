...
void Preprocess::give_feature(pcl::PointCloud<PointType> &pl, vector<orgtype> &types)
{
  ...
  Eigen::Vector3d curr_direct(Eigen::Vector3d::Zero()); // 当前平面方向
  ...
  {
    if (types[i].range < blind) // 在盲区（最小值）范围内的点不做处理
    {
      continue;
    }
  ...
  for (uint i = head + 3; i < plsize2; i++)
  {
    ...
    for (int j = 0; j < 2; j++)
    {
      ...
      */
      ...
      if (types[i].angle[j] < jump_up_limit) // cos(170度)
      {
